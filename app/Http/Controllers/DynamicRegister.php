    ...
    public function store(Request $request)
{
    try {
        ...
        // Redirect back with errors and custom message
        return redirect()->back()->withErrors($e->validator)->withInput()->with('register-form', 'Register Form Field');
    }
    ...
    public function resendOtp(Request $request)
    {
        ...
        if (!$email) {
            return redirect()->back()->withErrors(['message' => 'No email found in the session.', 'otp-form' => 'OTP Form']);
        }
        ...
        // Redirect back to the route with info
        return redirect()->back()->with(['otp-form' => 'OTP Form', 'success' => 'OTP sent successfully!']);
    }
    ...
    public function registerVerifyOtpStore(Request $request)
    {
        ...
        if ($redirectTo === 'back') {
            return redirect()->back()->with(['regsuccess' => 'Welcome to 900ticket', 'firstname' => $firstname, 'lastname' => $lastname]);
        }

        return redirect()->route($redirectTo)->with(['regsuccess' => 'Welcome to 900ticket', 'firstname' => $firstname, 'lastname' => $lastname]);
    }
...
/* Route::post('/register', [RegisterWithOtpController::class, 'store'])->defaults('redirectRoute', 'custom.route.name')->defaults('sessionData', [
    'register_firstname' => 'John',
    'register_lastname' => 'Doe',
    'register_address' => '123 Main St',
    'register_email' => 'john@example.com',
    'register_phone' => '1234567890',
    'register_password' => Hash::make('password123')
]); */